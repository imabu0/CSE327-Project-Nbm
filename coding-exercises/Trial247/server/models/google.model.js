  ...
  async getAvailableStorage() {
    ...
    for (const token of storedTokens) {
      try {
        ...
        const available = 100000; // change later
  ...
  async downloadFile(fileId, destination) {
    try {
      ...
      if (!storedTokens || storedTokens.length === 0) {
        throw new Error("❌ No stored Google accounts.");
      }
  
      const token = storedTokens[0]; // Select the first valid token
      if (!token.access_token || !token.refresh_token) {
        throw new Error("❌ Missing credentials for the selected token.");
      }
  
      // ✅ Set credentials before making the request
      this.oauth2Client.setCredentials({
        access_token: token.access_token,
        refresh_token: token.refresh_token,
        expiry_date: token.expiry_date,
      ...
      if (!response || !response.data) {
        throw new Error("❌ No response data received from Google Drive.");
      }
  
      console.log("📥 Google Drive Response: File stream received.");
